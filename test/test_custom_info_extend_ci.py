...
def test_save_load_with_extend_extra_info(tmp_path: Path):
    ...
    df = pd.DataFrame(np.random.randn(4, 3), columns=["pet hamsters", "wild hamsters", "space hamsters"])
    ...
    extra_info = FITestExtraInfo(author="Spud", source="Potato", hamster_collective="going nuts")
    ...
    assert isinstance(metainfo_reload.custom_info.col_units["unit_cur_wild"], FITestUnit)
    assert isinstance(metainfo_reload.custom_info.col_units["unit_cur_space"], FITestUnit)